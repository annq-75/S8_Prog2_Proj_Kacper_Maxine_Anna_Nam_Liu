...
public class ClientPacketProcessor {
	private transient ClientMsg client;

	public ClientPacketProcessor(ClientMsg c) {
		this.client = c;
	}
	...
	public void process(Packet p) {
		...
		System.out.println("Buf =" + t);

		System.out.println("Server received packet with data length = " + p.data.length);// отладка
		System.out.println("Buf = client");
		System.out.println("Raw bytes: " + java.util.Arrays.toString(p.data));// отладка
		// System.out.println("Received raw data: " + Arrays.toString(p.data));//отладка

		byte type = buf.get();

		// destination 0 -> serveur?
		if (type == 1) {
			// création groupe coté client
			// besoin de passer en parametre le nombre de personne et leurs id pour
			// l'instant
		...
		/*
		 * else if (type == 3) { // FR : Envoi des utilisateurs hors ligne -- RU: Список
		 * пользователей офлайн sendOfflineUsers(p.srcId); }
		 */
		else if (type == 4) {// Anna
								// FR : Envoi des groupes de l'utilisateur -- RU: Список групп, где пользователь
								// участник
		...
		} else if (type == 12) { // Version-liu ---sorry redacted a bit, all was red, Anna

			// FR : Envoi des groupes créés par l'utilisateur -- RU: Список групп, созданных
			// пользователем
...
// ----------- Envoi des utilisateurs en ligne -----------Anna
	private void sendOnlineUsers(int requesterId) {// Здесь мы просто проходим по всем пользователям UserMsg, проверяем
													// isConnected(), исключаем самого себя, и собираем строку вида
													// "2,3,5,".
		StringBuilder sb = new StringBuilder();

		for (Map.Entry<Integer, UserMsg> entry : server.getUsers().entrySet()) {
			UserMsg user = entry.getValue();
			if (user.isConnected() && user.getId() != requesterId) {
				sb.append(user.getId()).append(",");
			}
		}

		System.out.println("DEBUG: Online users sent = " + sb); // temp -- debug-- временный вывод
		sendTextResponse(requesterId, sb.toString());
	}

// ------------------ old vers ---------------
	/*
	 * private void sendOnlineUsers(int requesterId) { StringBuilder sb = new
	 * StringBuilder(); for (UserMsg u : server.getUsers().values()) { if
	 * (u.isConnected()) { sb.append(u.getId()).append(","); } }
	 * sendTextResponse(requesterId, sb.toString()); }
	 */

// ----------- Envoi des utilisateurs hors ligne -----------
	/*
	 * private void sendOfflineUsers(int requesterId) { StringBuilder sb = new
	 * StringBuilder(); for (UserMsg u : server.getUsers().values()) { if
	 * (!u.isConnected()) { sb.append(u.getId()).append(","); } }
	 * sendTextResponse(requesterId, sb.toString()); }
	 */
...
// ----------- Méthode d'envoi d'un message texte générique -----------
	private void sendTextResponse(int destId, String message) {// Метод, который сервер использует для отправки ответа
																// клиенту
...
// --------------------------------------------------------------------
// faut prendre en compte la lecture de paquets -> décider si on lit les
// informations
...
// -----------------méthode ajout membre dans groupe--------------
	public void addMember(int userId, GroupMsg groupe, UserMsg user) {
		...
		// l'utilisateur est un propriétaire et que l'utilisateur n'est pas dans le
		// groupe
...
// ----------------------méthode envoie de message
	public void sendMessage(UserMsg target, GroupMsg groupTarget, Packet message) {
		// possbilité que target ou group target soit nul selon si on envoie a un groupe
		// ou une personne en particulier??
	}

// ----------------------méthode envoie de message stocké------------------
	public void sendStock() {
		// recherche périodique dans la base de données à chaque reconnexion des
		// utilisateurs
		// -> utilisateur(1) se connecte -> le serveur recherche dans la base de données
		// les messages qui ne n'ont pas été envoyés -> si il y en a ou le destId ==
		// userId alors on envoie
		// -> utilisation de
	}

// ----------------------les méthodes de liu-----------------------------sorry redacted a bit, all was red, Anna
// Version- Liu
// ----------- Envoi des groupes créés par l'utilisateur ----------

	private void sendCreatedGroups(int userId) {
		Set<GroupMsg> createdGroups = new HashSet<>();
		for (GroupMsg group : server.getGroups().values()) { // 遍历服务器上的所有群组
